    ...
    >
      ...
      >
        <div className="flex items-baseline gap-4 mb-8">
          ...
          <span className="font-mono text-[11px] uppercase tracking-[0.22em] text-slate-brand">
            What Investors Will Eventually Value
          </span>
          <span className="hidden md:block flex-1 h-px bg-ink/15 ml-6" />
        </div>
        ...
        <div className="grid lg:grid-cols-12 gap-6 md:gap-8">
          {/* NOT card */}
          <div className="lg:col-span-4 bg-cream border border-line p-7 md:p-8 flex flex-col gap-5">
            <div className="text-[10px] uppercase tracking-[0.22em] text-slate-brand">
              Investors will NOT care only about
            </div>
            <div className="w-16 h-16 rounded-full border border-ink/20 flex items-center justify-center">
              <X className="w-5 h-5" strokeWidth={1.6} />
            </div>
            <h3 className="font-editorial text-[26px] md:text-[30px] leading-[1.05] tracking-tight italic font-editorial-soft line-through decoration-orange-brand decoration-[2px] text-ink/70">
              Number of schools.
            </h3>
            <p className="text-[13px] uppercase tracking-[0.22em] text-slate-brand mt-auto">
              A vanity metric
            </p>
          </div>

          {/* THEY WILL card */}
          <div className="lg:col-span-8 bg-orange-brand text-paper p-7 md:p-10 relative overflow-hidden">
            <div
              aria-hidden
              className="pointer-events-none absolute -bottom-16 -right-4 font-editorial italic text-[200px] leading-none text-paper/[0.08] select-none"
            >
              $
            </div>
            <div className="relative z-10 text-[10px] uppercase tracking-[0.22em] text-paper/80 mb-6">
              They will care about
            </div>
            <div className="relative z-10 grid sm:grid-cols-2 gap-3">
              {investorValues.map((v, i) => (
                <motion.div
                  key={v.k}
                  initial={{ opacity: 0, y: 10 }}
                  whileInView={{ opacity: 1, y: 0 }}
                  ...
                  data-testid={`investor-value-${i}`}
                >
                  <div className="w-9 h-9 rounded-full bg-paper/15 flex items-center justify-center shrink-0">
                    <v.icon className="w-4 h-4" strokeWidth={1.6} />
                  </div>
                  <div>
                    <div className="font-editorial text-[16px] leading-tight tracking-tight">
                      {v.k}
                    </div>
                    <div className="text-[11.5px] leading-[1.45] opacity-85 mt-0.5">
                      {v.v}
                    </div>
                  </div>
                </motion.div>
